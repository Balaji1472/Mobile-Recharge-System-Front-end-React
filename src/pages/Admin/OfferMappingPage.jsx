    ...
    <div className="om-overlay" onClick={!loading ? onClose : undefined}>
      <div className="om-confirm-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="om-confirm-actions">
          <button className="om-btn-cancel" onClick={onClose} disabled={loading}>
            Cancel
          </button>
...
export default function OfferMappingPage() {
  ...
  const [mapping, setMappingLoading2] = useState(false);  // submit in progress
  ...
  /* ── fetch plans + active offers for dropdowns ── */
  useEffect(() => {
    const fetchMeta = async () => {
      try {
        ...
        const msg = err.response?.data?.message || 'Failed to load plans/offers';
        showToast(msg, 'error');
      } finally {
  ...
  /* ── fetch all current mappings ──
     Strategy: fetch mappings per plan and flatten into one list            */
  useEffect(() => {
    const fetchAllMappings = async () => {
      try {
        ...
        const results = await Promise.allSettled(
          allPlans.map((p) => api.get(`/plans/${p.planId}/offers`))
        );
  ...
  const handleMap = async () => {
    ...
    setMappingLoading2(true);
    try {
      const { data } = await api.post(`/plans/${form.planId}/offers`, {
        offerId:  Number(form.offerId),
        priority: Number(form.priority),
      });
      showToast('Offer mapped to plan successfully!', 'success');
      setMappings((prev) => [...prev, data]);
      setForm({ planId: '', offerId: '', priority: '' });
      setErrors({});
    } catch (err) {
      const msg = err.response?.data?.message || 'Failed to map offer';
      showToast(msg, 'error');
    } finally {
      setMappingLoading2(false);
    }
  };
  ...
  const handleUnmap = async () => {
    ...
    try {
      await api.delete(`/plans/${unmapTarget.planId}/offers/${unmapTarget.offerId}`);
      showToast('Offer unmapped from plan successfully!', 'success');
      setMappings((prev) => prev.filter((m) => m.planOfferId !== unmapTarget.planOfferId));
      setUnmapTarget(null);
    } catch (err) {
      const msg = err.response?.data?.message || 'Failed to unmap offer';
      showToast(msg, 'error');
    } finally {
      setUnmappingId(null);
    }
      ...
      <div className="om-page">
        ...
        <div className="om-card">
          ...
          <div className="om-card-heading">
            ...
            {!mappingsLoading && (
              <span className="om-count">( {filtered.length} )</span>
            )}
          </div>
                    ...
                    <tbody>
                      {paginated.map((m) => (
                        <tr key={m.planOfferId}>
                          ...
                          <td>
                            <span className="om-priority-badge">P{m.priority}</span>
                          </td>
                          <td>
              ...
              {totalPages > 1 && (
                <div className="om-pagination">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
                    <button
                      key={n}
                      className={`om-pg-btn ${currentPage === n ? 'om-pg-btn--active' : ''}`}
                      onClick={() => setPage(n)}
                    >
                      {n}
                    </button>
