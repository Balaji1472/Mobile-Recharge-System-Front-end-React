      ...
      <div className="rfd-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rfd-modal-body">
          ...
          <div className="rfd-detail-grid">
            <DetailRow icon="fa-user"                  label="User"          value={refund.fullName || '—'} />
            <DetailRow icon="fa-mobile"                label="Mobile"        value={refund.mobileNumber} />
            <DetailRow icon="fa-layer-group"           label="Plan"          value={refund.planName} />
            <DetailRow icon="fa-indian-rupee-sign"     label="Amount"        value={formatAmount(refund.amount)} />
            <DetailRow icon="fa-calendar-check"        label="Processed At"  value={formatDateTime(refund.processedAt)} />
          </div>
          ...
          <div className="rfd-detail-grid">
            <DetailRow icon="fa-credit-card"  label="Payment ID"   value={`#PAY${String(refund.paymentId).padStart(5, '0')}`} />
            <DetailRow icon="fa-receipt"      label="Recharge ID"  value={`#RCH${String(refund.rechargeId).padStart(5, '0')}`} />
            <DetailRow icon="fa-wallet"       label="Method"       value={refund.paymentMethod} />
          </div>

          <hr className="rfd-modal-divider" />

          {/* Auto-trigger info note */}
          <div className="rfd-auto-note">
            <i className="fa-solid fa-circle-info"></i>
            <p>
              This refund was <strong>auto-triggered</strong> by the system because the
              payment was successful but the recharge failed.
            </p>
          </div>

          <div className="rfd-modal-actions">
            <button className="rfd-btn-cancel" onClick={onClose}>Close</button>
          </div>
        </div>
      </div>
    </div>
...
function exportCSV(data) {
  const headers = [
    'Refund ID', 'Payment ID', 'Recharge ID', 'User',
    'Mobile', 'Plan', 'Payment Method', 'Amount', 'Status', 'Processed At',
  ];
...
export default function RefundsPage() {
  ...
  useEffect(() => {
    const fetchRefunds = async () => {
      try {
        ...
        setRefunds(res.data);
        showToast('Refunds loaded successfully', 'success');
      } catch (err) {
        const msg = err.response?.data?.message || 'Failed to load refunds';
        setError(msg);
        showToast(msg, 'error');
      } finally {
        setLoading(false);
  ...
  const filtered = useMemo(() => {
    ...
    return refunds.filter((r) =>
      `rfd${String(r.refundId).padStart(5, '0')}`.includes(q) ||
  ...
  const handleExport = () => {
    if (filtered.length === 0) {
      showToast('No refunds to export', 'error');
      return;
    }
    exportCSV(filtered);
    showToast('Refunds CSV exported successfully', 'success');
  };
    ...
    <div>
      <div className="rfd-page">
        ...
        <div className="rfd-header">
          ...
          <p className="rfd-subtitle">All auto-generated refunds from failed recharge transactions</p>
        </div>
        ...
        {/* Info Banner — auto-refund note */}
        {!loading && !error && (
          <div className="rfd-info-banner">
            <i className="fa-solid fa-circle-info"></i>
            <span>
              Refunds are <strong>automatically triggered</strong> by the system when a
              payment is successful but the recharge fails. No manual action is needed.
            </span>
          </div>
        )}
        ...
        <div className="rfd-card">
          ...
          <div className="rfd-card-top">
            <div className="rfd-card-heading">
              ...
              {!loading && <span className="rfd-count">( {filtered.length} )</span>}
            </div>
            <button className="rfd-export-btn" onClick={handleExport} disabled={loading}>
          ...
          <div className="rfd-search-wrap">
            ...
            />
            {search && (
              <button className="rfd-search-clear" onClick={() => setSearch('')}>
                <i className="fa-solid fa-xmark"></i>
              </button>
            )}
          </div>
                    ...
                    <tbody>
                      {paginated.map((refund) => (
                        <tr key={refund.refundId}>
                          ...
                          <td>
                            <div className="rfd-td-user">
                              <div className="rfd-user-avatar">
                                {refund.fullName
                                  ? refund.fullName.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2)
                                  : '?'}
                              </div>
                              <div>
                                <p className="rfd-user-id">{refund.fullName || '—'}</p>
                                <p className="rfd-user-mobile">{refund.mobileNumber}</p>
                              </div>
                            </div>
                          </td>
                          <td className="rfd-td-plan">{refund.planName || '—'}</td>
                          <td className="rfd-td-amount">{formatAmount(refund.amount)}</td>
                          <td>
                          ...
                          <td className="rfd-td-date">{formatDate(refund.processedAt)}</td>
                          <td>
                            <button
