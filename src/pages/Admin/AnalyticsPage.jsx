...
export default function AnalyticsPage({ sidebarOpen, onSidebarClose }) {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        // Better error messaging for 403 vs other errors
    ...
    if (accessToken) {
      ...
      setError("No session found. Please login as admin.");
      setLoading(false);
    }
  }, [accessToken]);
            ...
            <div className="an-section-grid">
              <div className="an-card">
                ...
                <div className="an-stat-list">
                  <StatRow icon="fa-circle-check" label="Successful" value={formatNumber(data.successfulRecharges)} valueClass="green" bar={data.totalRecharges ? (data.successfulRecharges / data.totalRecharges) * 100 : 0} barClass="green" />
                  <StatRow icon="fa-circle-xmark" label="Failed" value={formatNumber(data.failedRecharges)} valueClass="red" bar={data.totalRecharges ? (data.failedRecharges / data.totalRecharges) * 100 : 0} barClass="red" />
                  <StatRow icon="fa-rotate-left" label="Refunded" value={formatNumber(data.refundedRecharges)} valueClass="orange" bar={data.totalRecharges ? (data.refundedRecharges / data.totalRecharges) * 100 : 0} barClass="orange" />
                </div>
              </div>
...
/* ── Sub-components (Kept inside for simplicity) ── */
