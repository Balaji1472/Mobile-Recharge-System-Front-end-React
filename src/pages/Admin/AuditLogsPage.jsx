...
function DetailRow({ icon, label, value, children }) {
  return (
    <div className="al-detail-row">
      <span className="al-detail-label">
        <i className={`fa-solid ${icon}`}></i>
        {label}
      </span>
      <span className="al-detail-value">{children ?? value ?? '—'}</span>
    </div>
  );
}
    ...
    <div className="al-overlay" onClick={onClose}>
      <div className="al-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="al-modal-body">
          <div className="al-modal-badge-row">
...
export default function AuditLogsPage() {
  ...
  const handleExport = () => {
    const rows = [
      ...
      ...filtered.map(l => [l.entityName, l.entityId, l.action, l.performedBy, l.oldValue || '', l.newValue || '', formatDateTime(l.timestamp)]),
    ];
    ...
    <div className="al-page">
      ...
      <div className="al-card">
        ...
        <div className="al-filters-row">
          <select className="al-select" value={actionFilter} onChange={handleFilter(setActionFilter)}>
            {actions.map(a => <option key={a} value={a}>{a === 'ALL' ? 'All Actions' : a}</option>)}
          </select>
          <select className="al-select" value={entityFilter} onChange={handleFilter(setEntityFilter)}>
          ...
          <>
                    ...
                    <tr key={log.id ?? log.logId ?? i}>
                      ...
                      <td>
                        <span className="al-user-chip">
                          <i className="fa-solid fa-user"></i> #{log.performedBy}
                        </span>
                      </td>
                      ...
                      <td>
                        <button className="al-view-btn" onClick={() => setSelectedLog(log)}>
            ...
            <div className="al-table-footer">
              <p className="al-count-text">
                Showing {(page - 1) * ITEMS_PER_PAGE + 1}–{Math.min(page * ITEMS_PER_PAGE, filtered.length)} of {filtered.length} logs
              </p>
