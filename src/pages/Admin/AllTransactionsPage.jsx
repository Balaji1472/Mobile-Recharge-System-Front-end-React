      ...
      <div className="txn-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="txn-modal-body">
          ...
          <div className="txn-modal-id-row">
            ...
            <div>
              <p className="txn-modal-txnid">{txn.transactionReference || '—'}</p>
          ...
          <div className="txn-detail-grid">
            <DetailRow icon="fa-user"        label="User"          value={txn.fullName || '—'} />
          ...
          <div className="txn-detail-grid">
            ...
            <DetailRow icon="fa-fingerprint"  label="Ref No."       value={txn.transactionReference || '—'} />
          ...
          <div className="txn-modal-actions">
            <button className="txn-btn-cancel" onClick={onClose}>Close</button>
          </div>
        </div>
      </div>
    </div>
...
function exportCSV(data) {
  const headers = [
    'Transaction Ref', 'User', 'Mobile', 'Plan', 'Amount',
    'Payment Method', 'Recharge Status', 'Payment Status', 'Date',
  ];
...
export default function AllTransactionsPage() {
  ...
  /* ── stat counts ── */
  const stats = useMemo(() => ({
    success:  transactions.filter((t) => t.rechargeStatus === 'SUCCESS').length,
    pending:  transactions.filter((t) => t.rechargeStatus === 'PENDING').length,
    failed:   transactions.filter((t) => t.rechargeStatus === 'FAILED').length,
    refunded: transactions.filter((t) => t.rechargeStatus === 'REFUNDED').length,
  }), [transactions]);
  ...
  const filtered = useMemo(() => {
    ...
    return transactions.filter((t) => {
      const matchStatus = statusFilter === 'ALL' || t.rechargeStatus === statusFilter;
      const matchSearch =
        !q ||
    ...
    <div>
      <div className="txn-page">
        ...
        <div className="txn-header">
          ...
          <p className="txn-subtitle">View and manage all platform transactions</p>
        </div>

        {/* Stat Cards */}
        {!loading && !error && (
          <div className="txn-stats-row">
            <StatCard icon="fa-circle-check"          count={stats.success}  label="Successful" colorClass="txn-stat--success" />
            <StatCard icon="fa-hourglass-half"        count={stats.pending}  label="Pending"    colorClass="txn-stat--pending" />
            <StatCard icon="fa-circle-xmark"          count={stats.failed}   label="Failed"     colorClass="txn-stat--failed"  />
            <StatCard icon="fa-rotate-left"           count={stats.refunded} label="Refunded"   colorClass="txn-stat--refunded"/>
          </div>
        )}
        ...
        <div className="txn-card">
          ...
          <div className="txn-card-top">
            <div className="txn-card-heading">
              ...
              {!loading && <span className="txn-count">( {filtered.length} )</span>}
            </div>
            <button className="txn-export-btn" onClick={handleExport} disabled={loading}>
                          ...
                          <td>
                            <div className="txn-td-user">
                              <div className="txn-user-avatar">
                                {txn.fullName
                                  ? txn.fullName.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2)
                                  : '?'}
                              </div>
                              <div>
                                <p className="txn-user-id">{txn.fullName || '—'}</p>
                                <p className="txn-user-mobile">{txn.mobileNumber}</p>
                              </div>
                            </div>
                          </td>
                          <td className="txn-td-plan">{txn.planName || '—'}</td>
                          <td className="txn-td-amount">{formatAmount(txn.finalAmount)}</td>
                          <td><MethodBadge method={txn.paymentMethod} /></td>
                          <td><StatusBadge status={txn.rechargeStatus} /></td>
                          <td className="txn-td-date">{formatDate(txn.initiatedAt)}</td>
                          <td>
                            <button
