...
import { loadOffers, editOffer, toggleOffer, terminateOffer, reset } from '../slice/offersSlice';
import { Spinner } from '../../../../components/common';
import { useToast } from '../../../../hooks/useToast';
import '../css/AllOffersPage.css';
...
function EditOfferModal({ offer, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const updated = await dispatch(editOffer({
      ...
      toast(err || 'Failed to update offer', 'error');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="ao-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="ao-form-grid">
          ...
          <div className="ao-field">
            <label className="ao-label">Discount Type <span>*</span></label>
            <select
              className={`ao-select${errors.discountType ? ' error' : ''}`}
            ...
            {errors.discountType && <span className="ao-error-text">{errors.discountType}</span>}
          </div>
    ...
    <div className="ao-overlay" onClick={!loading ? onClose : undefined}>
      <div className="ao-confirm-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="ao-confirm-msg">
          You're about to end <strong>"{offer.title}"</strong>. This will deactivate it immediately.
        </p>
        <div className="ao-confirm-actions">
          <button className="ao-btn-cancel" onClick={onClose} disabled={loading}>Cancel</button>
...
export default function AllOffersPage() {
  ...
  const { data: offers, isLoading, isError, message } = useSelector((state) => state.offers);
  ...
  const handleToggle = async (offer) => {
    ...
    try {
      await dispatch(toggleOffer(offer)).unwrap();
      const label = offer.active ? 'deactivated' : 'activated';
      toast(`Offer ${label} successfully!`, 'success');
    } catch (err) {
  ...
  const handleEnd = async () => {
    ...
    try {
      await dispatch(terminateOffer(endTarget.offerId)).unwrap();
      toast('Offer ended successfully!', 'success');
      setEndTarget(null);
    } catch (err) {
      toast(err || 'Failed to end offer', 'error');
    } finally {
      setEndingId(null);
    }
  };
  ...
  return (
    <div>
        ...
        <div className="ao-card">
          ...
          <div className="ao-card-top">
            <div className="ao-card-heading">
              ...
              {!isLoading && <span className="ao-count">( {filtered.length} )</span>}
            </div>
            ...
            <>
                        ...
                        <tr key={offer.offerId}>
                          ...
                          <td>
                            <div className="ao-actions">
                              <button className="ao-btn-edit" onClick={() => setEditOffer(offer)}>
                              ...
                              {offer.active && (
                                <button className="ao-btn-end" onClick={() => setEndTarget(offer)}>
              ...
              {totalPages > 1 && (
                <div className="ao-pagination">
                  <button className="ao-pg-btn" disabled={currentPage === 1} onClick={() => setPage((p) => p - 1)}>
                    <i className="fa-solid fa-chevron-left" />
                  </button>
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
                    <button key={n} className={`ao-pg-btn ${currentPage === n ? 'ao-pg-btn--active' : ''}`} onClick={() => setPage(n)}>
                      {n}
                    </button>
                  ))}
                  <button className="ao-pg-btn" disabled={currentPage === totalPages} onClick={() => setPage((p) => p + 1)}>
      ...
      {editOffer && (
        <EditOfferModal offer={editOffer} onClose={() => setEditOffer(null)} onSaved={handleEditSaved} />
      )}
      {endTarget && (
        <ConfirmEndModal
          offer={endTarget}
